...
function utils.preprocess_us(path, mean_path, std_path)
  local img = image.load(path):mul(255)
  local mean_im =image.load(mean_path):mul(255);
  local std_im =image.load(std_path):mul(255);
  
  img=torch.cdiv((img-mean_im),std_im);
  return img, img:size(2), img:size(3);
end
...
function utils.create_grad_input_batch(module, labels)
  -- local 
  ...
  -- print (doutput:size());
  doutput:fill(0)
  for im_num=1,labels:size(1) do
    -- print (im_num,labels[im_num]);
    doutput[{im_num,labels[im_num]}] = 1
  end
  -- print (doutput:size());
  -- doutput= doutput:view(-1);
  return doutput
...
function utils.grad_cam_batch(cnn, layer_name, doutput)
  -- Split model into two
  local model1, model2 = nn.Sequential(), nn.Sequential()
  ...
  -- Global average pool gradients
  local weights = torch.sum(gradients:view(activations:size(1), activations:size(2),-1), 3)
  -- print ('weights:size()')
  -- print (weights:size());
  -- print (weights:view(activations:size(1),activations:size(2), 1, 1):size())
  -- print (weights:view(activations:size(1),activations:size(2), 1, 1):expandAs(activations):size())

  -- Summing and rectifying weighted activations across depth
  local map = torch.sum(torch.cmul(activations, weights:view(activations:size(1),activations:size(2), 1, 1):expandAs(activations)), 2)
  map = map:cmul(torch.gt(map,0):typeAs(map))
  -- print (map:size())
